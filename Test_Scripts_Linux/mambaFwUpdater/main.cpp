...
static void turnOffLEDs(UVCInterface* uvcMgr)
{
   unsigned char data[256];
   sendVendorCommand(uvcMgr, true, 0xDE, 0, 0, data, 0, 1);
}
...
int main(int argc, char* argv[]) {
   ...
   MambaImgType_T imgType = MAIN_APP;
   if (strcmp(argv[1], "boot") == 0) {
      imgType = BOOTLOADER;
   } else if (strcmp(argv[1], "main") == 0) {
      imgType = MAIN_APP;
   } else {
      printf("ERROR: First argument must be boot or main!\n");
      printUsageAndExit(argv[0]);
   }
...
#if 0
   unsigned char ready;
   if (!uvcMgr->getProperty(0x1, &ready, 1)) {
      printf("failed to query system ready!\n");
   }
   printf("system ready = %d\n", ready);
#endif

   int pageSize = getMambaPageSize(uvcMgr); //check fw update version
   int ret = mambaFwUpdate(uvcMgr, argv[2], pageSize, imgType); //flash image

   //TODO: reboot mamba. If mamba will be reboot, probably reset LED is not needed.
   turnOffLEDs(uvcMgr); //in case of update error, device keeps LED on
